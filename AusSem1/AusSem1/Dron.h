...
private:
	...
	// dolet podla stavu bateria == ((( ( rychlost_/60 )*dobaLetu_) / 2) * bateriaPercenta_) / 100
	int maxDolet_;
	int aktDolet_;
	double nalietaneHodiny_;

public:
	Dron(int sCislo, int typ, int nosnost, int rychlost, int dobaLetu, int dobaNabijania, Datum *datum, Objednavka *objednavka = nullptr, bool jeVSklade = true, int bateria = 100, double casD = 0, double casV = 0, double h = 0);
	~Dron();
	void initDolet();
